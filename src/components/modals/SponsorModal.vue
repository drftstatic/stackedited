<template>
  <modal-inner class="modal__inner-1--sponsor" aria-label="Sponsor">
    <div class="modal__content">
      ...
      <a v-for="button in buttons" :key="button.id" class="paypal-option button flex flex--row flex--center" :href="button.link">
        <div class="flex flex--column">
          <div>{{ button.price }}<div v-if="button.offer" class="paypal-option__offer">{{ button.offer }}</div></div>
